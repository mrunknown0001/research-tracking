  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="{{ route('admin.proceed.step.nine.post') }}" method="POST">
          {{ csrf_field() }}
          <div class="form-group">
            <input type="number" name="grade" id="grade" max="100" class="form-control" placeholder="UREC Grade" required>
          </div>
          <div class="form-group">
            <select name="agenda" id="agenda" class="form-control" required="">
              <option value="">Select Agenda</option>
              @foreach($agendas as $a)
                <option value="{{ $a->id }}">{{ ucwords($a->title) }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
